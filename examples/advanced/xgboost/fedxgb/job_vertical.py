...
def run_psi_job(args):
    ...
    # Run PSI job
    env = SimEnv(num_clients=args.site_num, workspace_root="/tmp/nvflare/workspace/works/xgboost_vertical_psi")
...
def run_training_job(args):
    ...
    # Run training
    env = SimEnv(num_clients=args.site_num, workspace_root="/tmp/nvflare/workspace/works/xgboost_vertical")
    run = recipe.execute(env)
