...
# Example of creating custom privacy filter
...
if __name__ == "__main__":
    ...
    recipe.add_client_output_filter(
        SVTPrivacy(fraction=0.1, epsilon=0.1, noise_var=0.1, gamma=1e-5, tau=1e-6, replace=True)
    )
    # recipe.add_client_output_filter(CustomPrivacy(percentile=15, gamma=0.02))

    # Define experiment
    # Run experiment
    recipe.execute(env=SimEnv(gpu="0", workdir="/tmp/nvflare/cifar10_fedavg_dp", clients=n_clients))
