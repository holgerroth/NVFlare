...
def train_model(
    ...
    # Setup the trainer
    # Note, instead of using llm.train() as in the bionemo finetune_esm2.py script, we break it up to call setup once and trainer.fit() for each round NVFlare is running.
    app_state = _setup(
        model=module,
        data=data_module,
        trainer=trainer,
