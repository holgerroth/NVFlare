...
## ---------------------------------------------------------------------------
##
## File: train_centralized.py for Newton Raphson
##
## Created by Zhijin Li
## E-mail:   <zhijinl@nvidia.com>
##
## Started on  Mon Apr 15 01:28:27 2024 Zhijin Li
## Last update Mon Apr 15 09:58:08 2024 Zhijin Li
## ---------------------------------------------------------------------------
...
if __name__ == '__main__':
  ...
  parser.add_argument(
    ...
    type=str,
    default='custom',
    help=('which solver to use: custom (default) or sklearn '
          'LogisticRegression. The results are the same. '))
...
for site in range(4):
  ...
  print('\nsite - {}'.format(site+1))
  test_X = np.load(
    os.path.join(DATA_ROOT, 'site-{}.test.x.npy'.format(site+1)))
  test_y = np.load(
    os.path.join(DATA_ROOT, 'site-{}.test.y.npy'.format(site+1)))
  ...
  if args.solver == 'sklearn':
    ...
    test_X = np.concatenate(
      (np.ones((test_X.shape[0], 1)), test_X),
      axis=1
    )
