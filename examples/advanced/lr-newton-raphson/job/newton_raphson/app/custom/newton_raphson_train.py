...
## ---------------------------------------------------------------------------
##
## File: newton_raphson_train.py for Newton Raphson
##
## Created by Zhijin Li
## E-mail:   <zhijinl@nvidia.com>
##
## Started on  Fri Apr 12 11:38:39 2024 Zhijin Li
## Last update Mon Apr 15 02:33:55 2024 Zhijin Li
## ---------------------------------------------------------------------------
...
def parse_arguments():
  ...
  parser = argparse.ArgumentParser(
    description="Federated Second-Order Newton Raphson"
  )

  parser.add_argument(
    '--data_root',
    type=str,
    help='Path to load client side data.'
  )
...
def sigmoid(inp): return 1.0 / (1.0 + np.exp(-inp))
...
def validate(data, theta):
  ...
  valid_X = np.concatenate(
    (np.ones((valid_X.shape[0], 1)), valid_X),
    axis=1
  )
  ...
  return {
    'accuracy': accuracy_score(valid_y, proba.round()),
    'precision': precision_score(valid_y, proba.round())
  }
...
def main():
  ...
  while flare.is_running():
    ...
    print(('[ROUND {}] - client site: {}, received '
           'global model: {}').format(
    ...
    print(('[ROUND {}] - start validation of global '
           'model on client: {}').format(
             curr_round, site_name))
    validation_scores = validate(data, global_weights)
    print(('[ROUND {}] - validation metric scores on '
           'client: {} = {}').format(
             curr_round, site_name, validation_scores))

    # Write validation metric summary
    writer.add_scalar(
      '{}/accuracy'.format(site_name),
      validation_scores['accuracy'],
      curr_round
    )

    writer.add_scalar(
      '{}/precision'.format(site_name),
      validation_scores['precision'],
      curr_round
    )

    # Local training
    print(('[ROUND {}] - start local training on client '
           'site: {}').format(
             curr_round, site_name))
    result_dict = train_newton_raphson(
      data,
      theta=global_weights
    )
    ...
    print(
      ('[ROUND {}] - local newton raphson training from '
       'client: {} complete, sending results to server: {}').format(
         curr_round, site_name, result_model))
