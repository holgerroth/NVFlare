...
class CIFAR10LearnerSplitNN(Learner):
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        if self.split_id == 1:
            ...
            #)
            engine.cell.register_request_cb(channel="abc", topic=SplitNNConstants.TASK_TRAIN_LABEL_STEP, cb=self._aux_train_label_side)
            #engine.register_aux_message_handler(
        ...
        #self.labels = torch.zeros(64, dtype=torch.long).to(self.device)
    ...
    def _aux_train_label_side(self, request):
    ...
    def train(self, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        #self.inputs = torch.rand(self.batch_size, 3, 32, 32).to(self.device)
        #self.train_size = self.batch_size
        #self.activations = torch.rand(self.batch_size, 4096).to(self.device)
        #self.gradient = torch.rand(self.batch_size, 4096).to(self.device)

        self.inputs = np.random.rand(self.batch_size, 3, 32, 32)
        self.train_size = self.batch_size
        self.activations = np.random.rand(self.batch_size, 4096)
        self.gradient = np.random.rand(self.batch_size, 4096)
        ...
        for _curr_round in range(self.num_rounds):
            ...
            result = engine.cell.send_request(
                channel="abc",
                topic=SplitNNConstants.TASK_TRAIN_LABEL_STEP,
                target=self.other_client+"."+job_id,
                request=new_message(payload=data_shareable),
                timeout=0.0
            )
            time.sleep(0.05)
