...
class FedExLora(BaseFedAvg):
    ...
    def fedexlora_aggregate_fn(results: List[FLModel]) -> FLModel:
        print("$$$$$$$ aggr models:", results)
    
        agg = self.aggregate_fn(results)
    ...
    def run(self) -> None:
        ...
        for self.current_round in range(self.start_round, self.start_round + self.num_rounds):
            ...
            aggregate_results = self.aggregate(
                results, aggregate_fn=self.fedexlora_aggregate_fn
            )  
