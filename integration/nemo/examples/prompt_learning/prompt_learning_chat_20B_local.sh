...
# create configs
job_name="gpt_p-tuning_local_20B_chat"

job_path="jobs/$job_name"
rm -r "$job_path"
python3 create_configs.py --job_folder="$job_path" --num_clients="$n_clients" --devices 2 --aggregation_epochs=50 --num_rounds=1 --train_ds_files_prefix="$data_dir/site-" --train_ds_files_suffix="-training.jsonl" --val_ds_files="$data_dir/site-0-validation.jsonl"
