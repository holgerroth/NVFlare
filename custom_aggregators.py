...
class FedAvgMRoundDecayAggregator(FedAvgMAggregator):
    """FedAvgM with linear decay of server_lr across rounds.

    Effective server_lr at round t is `start * (1 - t/(decay_horizon)) + end * (t/decay_horizon)`,
    clamped to [end, start]. server_momentum stays constant.
    """
    ...
    def __init__(
        ...
        server_lr: float = 1.0,
        server_momentum: float = 0.6,
        server_lr_end: float = 1.0,
        decay_horizon: int = 20,
    ):
        super().__init__(server_lr=server_lr, server_momentum=server_momentum)
        if decay_horizon <= 0:
            raise ValueError("decay_horizon must be > 0")
        self.server_lr_start = server_lr
        self.server_lr_end = server_lr_end
        self.decay_horizon = decay_horizon
        self._round_idx = 0

    def aggregate_model(self) -> FLModel:
        progress = min(1.0, self._round_idx / float(self.decay_horizon))
        self.server_lr = self.server_lr_start * (1.0 - progress) + self.server_lr_end * progress
