...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Client-side weight EMA decay. 0 disables EMA; effective range 0.9-0.999.",
    )
    parser.add_argument(
...
def main(args):
    ...
    if args.client_optimizer == "adamw":
        ...
        optimizer = optim.SGD(
            ...
            weight_decay=args.weight_decay,
            nesterov=(args.client_optimizer == "sgdn") and args.momentum > 0.0,
        )
    ...
    while flare.is_running():
        ...
        for epoch in range(args.aggregation_epochs):
            ...
            for batch in train_loader:
                ...
                def forward_loss():
                    out = model(inputs)
                    if args.mixup_alpha > 0.0:
                        return lam * criterion(out, labels) + (1.0 - lam) * criterion(out, labels[perm])
        ...
        if args.ema_decay > 0.0 and ema_state is not None:
