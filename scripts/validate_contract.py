...
REQUIRED_CHECKS = [
    ...
    "flare.is_evaluate() branch is present",
]
...
def contains_flare_send(node):
    for child in ast.walk(node):
        if not isinstance(child, ast.Call):
            continue
        if call_name(child.func) == "flare.send":
            return True
    return False
...
class ClientContractVisitor(ast.NodeVisitor):
    ...
    def visit_If(self, node):
        if contains_call(node.test, "flare.is_evaluate"):
            self.has_eval_diff_send = self.has_eval_diff_send or contains_flare_send(
