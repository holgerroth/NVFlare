...
class CollabExecutor(Executor, CollabAdaptor):
    """Collab Executor for client-side execution.

    Supports two execution modes:
    - inprocess=True (default): Execute @collab.publish methods in the same process
    - inprocess=False: Spawn a subprocess for execution (e.g., for torchrun DDP)

    When inprocess=False, the executor spawns a CollabWorker subprocess using the
    ...
    def _detect_client_class(self) -> Optional[str]:
        ...
        Returns:
            The class name if using a class-based client (e.g., "CollabClientAPI"),
    ...
    def _start_subprocess_worker(self, cell, client_name: str):
        ...
        # Detect client class for Client API mode (e.g., "CollabClientAPI")
