...
@flare.collab
def client_train(self, current_round, weights):
    result = {}
...
def main():
    ...
    recipe = FlareRecipe(
        name="pt_fedavg",
        server=FlareAlgo(algo_func=run_fedavg(initial_model=initial_model, num_rounds=4)),
        clients=[
            FlareCollab(collab_funcs=[client_train(delta=1.0)]),
            FlareCollab(collab_funcs=[get_stats()]),
        ],
    )
