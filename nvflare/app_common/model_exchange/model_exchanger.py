...
class ModelExchanger:
    ...
    def submit_model(self, model: Any) -> None:
        ...
        if self._req_id is None:
            raise DataExchangeException("Missing req_id, need to pull a model first.")

        if self.current_topic is None:
            raise DataExchangeException("Missing current_topic, need to pull a model first.")

        self._send_reply(data=model, topic=self.current_topic, req_id=self._req_id)
