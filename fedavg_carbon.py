...
class FedAvg(BaseFedAvg):
    ...
    def collect_emission_data(self, results):
        n_clients_round = max(1, len(results))
        for result in results:
            ...
            # carry over timing info and number of clients in this round
            emission["n_clients_round"] = n_clients_round
            try:
                emission["round_total_seconds"] = result.meta.get("ROUND_TOTAL_SECONDS")
                emission["train_seconds"] = result.meta.get("TRAIN_SECONDS")
                emission["eval_seconds"] = result.meta.get("EVAL_SECONDS")
            except Exception:
                pass
            ...
            # per-client normalization for train energy/emissions (CPU cross-check alignment)
            train = emission.get("train")
            try:
                t_energy = getattr(train, "energy_consumed", None)
            except Exception:
                t_energy = train.get("energy_consumed") if isinstance(train, dict) else None
            try:
                t_emiss = getattr(train, "emissions", None)
            except Exception:
                t_emiss = train.get("emissions") if isinstance(train, dict) else None
            if t_energy is not None:
                emission["train_energy_kwh_per_client"] = float(t_energy) / n_clients_round
            if t_emiss is not None:
                emission["train_emissions_kg_per_client"] = float(t_emiss) / n_clients_round
    ...
    def save_client_emissions(self):
        ...
        out_client_emissions = {
            ...
            "ram_energy": [],
            "energy_consumed": [],
            "train_energy_kwh_per_client": [],
            "train_emissions_kg_per_client": [],
        ...
        for client_name, emissions in self.client_emissions.items():
            for emission in emissions:
                ...
                out_client_emissions["round_total_seconds"].append(emission.get("round_total_seconds"))
                out_client_emissions["train_seconds"].append(emission.get("train_seconds"))
                out_client_emissions["eval_seconds"].append(emission.get("eval_seconds"))
                out_client_emissions["n_clients_round"].append(emission.get("n_clients_round"))
                ...
                if train is None:
                    ...
                    out_client_emissions["train_energy_kwh_per_client"].append(emission.get("train_energy_kwh_per_client"))
                    out_client_emissions["train_emissions_kg_per_client"].append(emission.get("train_emissions_kg_per_client"))
