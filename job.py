...
def define_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Cross-entropy label smoothing coefficient. 0 disables smoothing.",
    )
    parser.add_argument(
        "--grad_clip_norm",
        type=float,
        default=0.0,
        help="Maximum L2 grad norm for clip_grad_norm_. 0 disables clipping.",
    )
    parser.add_argument(
        "--client_optimizer",
        type=str,
        default="sgd",
        choices=["sgd", "sgdn", "adamw"],
        help="Client-side optimizer family: sgd, sgdn (SGD+Nesterov), adamw.",
    )
    parser.add_argument(
        "--mixup_alpha",
        type=float,
        default=0.0,
        help="MixUp Beta(alpha,alpha) mixing strength. 0 disables MixUp.",
    )
    parser.add_argument(
        "--sam_rho",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Client-side weight EMA decay. 0 disables EMA.",
    )
    parser.add_argument(
        "--reset_optimizer_each_round",
        action="store_true",
        help="Zero the client optimizer momentum buffers at the start of each round.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Endpoint server_lr for fedavgm_decay (decays from server_lr to server_lr_end).",
    )
    parser.add_argument(
        "--server_decay_horizon",
        type=int,
        default=20,
        help="Round horizon for fedavgm_decay linear decay.",
    )
...
def get_aggregator(args):
    ...
    if kind == "fedavgm_decay":
        print(
            "Using FedAvgMRoundDecayAggregator "
            f"(server_lr={args.server_lr}, server_lr_end={args.server_lr_end}, "
            f"server_momentum={args.server_momentum}, decay_horizon={args.server_decay_horizon})"
        )
        return FedAvgMRoundDecayAggregator(
            server_lr=args.server_lr,
            server_lr_end=args.server_lr_end,
            server_momentum=args.server_momentum,
            decay_horizon=args.server_decay_horizon,
        )
    if kind == "median":
...
def main():
    ...
    if args.reset_optimizer_each_round:
        train_args.append("--reset_optimizer_each_round")
