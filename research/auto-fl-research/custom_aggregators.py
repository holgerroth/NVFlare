...
class FedExpAggregator(ModelAggregator):
    """FedExP-style adaptive server step over weighted client DIFFs.

    The paper's extrapolated step is computed from client update diversity. The
    configured server_lr is used as a safety cap for this harness.
    """

    def __init__(self, max_server_lr: float = 3.0, epsilon: float = 1e-12):
        super().__init__()
        if max_server_lr < 1.0:
            raise ValueError("fedexp max_server_lr must be >= 1")
        if epsilon <= 0.0:
            raise ValueError("fedexp epsilon must be > 0")
        self.max_server_lr = max_server_lr
    ...
    def accept_model(self, model: FLModel):
        ...
        for key, value in model.params.items():
            ...
            if key not in self.weighted_sum:
                ...
                self.weighted_sum[key] += diff * weight
            client_norm_sq += float(np.sum(np.square(diff)))

        self.client_norm_sq_weighted_sum += weight * client_norm_sq
        self.total_weight += weight
    ...
    def aggregate_model(self) -> FLModel:
        ...
        mean_diff = {key: val / self.total_weight for key, val in self.weighted_sum.items()}
        mean_norm_sq = sum(float(np.sum(np.square(diff))) for diff in mean_diff.values())
        avg_client_norm_sq = self.client_norm_sq_weighted_sum / self.total_weight
        fedexp_lr = max(1.0, avg_client_norm_sq / (mean_norm_sq + self.epsilon))
        fedexp_lr = min(self.max_server_lr, fedexp_lr)

        aggregated_params = {
            key: _to_output_type(fedexp_lr * mean_diff[key], self.references[key]) for key in mean_diff
        }
        return FLModel(params=aggregated_params, params_type=self.params_type)
    ...
    def reset_stats(self):
        self.weighted_sum = {}
        self.total_weight = 0.0
        self.client_norm_sq_weighted_sum = 0.0
        self.client_weights = []
        self.params_type = None
        self.references = {}
