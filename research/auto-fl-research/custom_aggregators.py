...
class AlignedFedAvgMAggregator(FedOptAggregator):
    """FedAvgM with client DIFF weights modulated by update alignment.

    The base direction is the step-weighted mean DIFF for the round. Client updates
    that point in a similar direction receive higher effective weight, while a
    floor prevents a single round from fully dropping heterogeneous clients.
    """
    ...
    def __init__(
        ...
        if not 0.0 <= alignment_floor <= 1.0:
            raise ValueError("alignment_floor must be in [0, 1]")
    ...
    def accept_model(self, model: FLModel):
        ...
        client_params = {}
        for key, value in model.params.items():
            client_params[key] = _as_numpy(value).astype(np.float64, copy=False)
            self.references.setdefault(key, value)
        self.client_models.append(client_params)
    ...
    def aggregate_model(self) -> FLModel:
        ...
        base_weights = np.asarray(self.client_weights, dtype=np.float64)
        if np.sum(base_weights) <= 0.0:
            self.error("Total weight is zero, cannot aggregate")
            return FLModel(params={})

        base_mean = self._weighted_mean(base_weights)
        mean_norm_sq = sum(float(np.sum(np.square(value))) for value in base_mean.values())
        mean_norm = np.sqrt(mean_norm_sq)

        alignment_weights = []
        for client_params, base_weight in zip(self.client_models, base_weights):
            client_norm_sq = 0.0
            dot = 0.0
            for key, mean_value in base_mean.items():
                client_value = client_params[key]
                client_norm_sq += float(np.sum(np.square(client_value)))
                dot += float(np.sum(client_value * mean_value))

            client_norm = np.sqrt(client_norm_sq)
            if mean_norm <= 0.0 or client_norm <= 0.0:
                alignment = 1.0
            else:
                alignment = dot / (client_norm * mean_norm + 1e-12)
                alignment = min(1.0, max(self.alignment_floor, alignment))
            alignment_weights.append(base_weight * alignment)

        alignment_weights = np.asarray(alignment_weights, dtype=np.float64)
        if np.sum(alignment_weights) <= 0.0:
            alignment_weights = base_weights

        mean_diff = self._weighted_mean(alignment_weights)
        update = self._sgdm_update(mean_diff)
        aggregated_params = {key: _to_output_type(update[key], self.references[key]) for key in update}
        return FLModel(params=aggregated_params, params_type=self.params_type)

    def _weighted_mean(self, weights):
        total_weight = float(np.sum(weights))
        result = {}
        for key in self.client_models[0].keys():
            weighted_sum = None
            for client_params, weight in zip(self.client_models, weights):
                value = client_params[key] * weight
