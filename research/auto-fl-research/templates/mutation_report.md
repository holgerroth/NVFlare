...
Class-balanced loss failed and was reverted; post-mixup epoch-count, server-momentum, client-LR, and FedDyn-alpha audits regressed or failed to clear the material threshold. Next CLI-only continuation should retune the existing `feddrift_mu` magnitude under mixup.
