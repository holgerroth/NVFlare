...
Class-balanced loss failed and was reverted; post-mixup epoch-count, server-momentum, and client-LR audits regressed or failed to clear the material threshold. Next CLI-only continuation should retune an existing drift-regularization scalar under mixup, starting with `feddyn_alpha`.
