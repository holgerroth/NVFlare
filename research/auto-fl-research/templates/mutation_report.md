...
- Post-mixup server-momentum retune did not improve: `server_momentum=0.40` scored `0.911000`; `0.30` scored `0.910300`. Keep `server_momentum=0.35`.
- Post-mixup client-LR sweep produced a near miss but no material keep: `lr=0.055` scored `0.914500`, only `0.000400` above the kept best and below the `0.000500` threshold; `lr=0.045` scored `0.910700`.
- Narrowing around the LR near miss did not improve: `lr=0.0525` scored `0.912400`; `0.0575` scored `0.910600`. Do not adopt `lr=0.055` without a material confirmation.
- Post-mixup FedDyn-alpha retune did not improve: `feddyn_alpha=5e-5` scored `0.910200`; `2e-4` scored `0.909800`. Keep `feddyn_alpha=1e-4`.
- Post-mixup FedDrift-mu retune did not improve: `feddrift_mu=3.75e-5` scored `0.912800`; `1.25e-5` scored `0.909800`. Keep `feddrift_mu=2.5e-5`.
- Post-mixup FedDrift-beta retune did not improve: `feddrift_beta=0.8` scored `0.912200`; `0.95` scored `0.911400`. Keep `feddrift_beta=0.9`.
- Drift regularizer ablations under mixup did not improve: removing FedDrift scored `0.912500`; removing FedDyn scored `0.911000`. Keep both `feddyn_alpha=1e-4` and `feddrift_mu=2.5e-5`.
- Scheduler-floor retry under mixup improved the best: `cosine_lr_eta_min_factor=0.003` scored `0.915600`; `0.03` regressed to `0.906900`. Keep `cosine_lr_eta_min_factor=0.003`.
- Scheduler-floor/client-LR interaction improved again: `lr=0.0475` scored `0.916200`; `lr=0.055` regressed to `0.914000`. Keep `lr=0.0475`.
- Server-LR retune under the `lr=0.0475`, `eta_min_factor=0.003` best stack regressed: `server_lr=1.9375` scored `0.914100`; `1.8125` scored `0.913800`. Keep `server_lr=1.875`.
- Server-momentum retune under the same stack regressed: `server_momentum=0.40` scored `0.914600`; `0.30` scored `0.911600`. Keep `server_momentum=0.35`.
- Weight-decay retune under the same stack regressed: `weight_decay=3.25e-4` scored `0.914200`; `3.75e-4` scored `0.909900`. Keep `weight_decay=3.5e-4`.
- FedDyn-alpha retune under the same stack regressed: `feddyn_alpha=5e-5` scored `0.915000`; `2e-4` scored `0.910500`. Keep `feddyn_alpha=1e-4`.
...
- SGDR / cosine schedules: Ilya Loshchilov, Frank Hutter. "SGDR: Stochastic Gradient Descent with Warm Restarts." ICLR 2017; arXiv:1608.03983.
...
The calibration result now favors FedNova-style normalized DIFF aggregation plus small client-local drift corrections and local-only mixup with the original `moderate_cnn` architecture. The best kept stack is `--aggregator fednova`, `server_lr=1.875`, `server_momentum=0.35`, client `lr=0.0475`, default client momentum, epoch-based local training with `aggregation_epochs=5`, `weight_decay=3.5e-4`, `--gradient_centralization`, `--feddyn_alpha 1e-4`, `--feddrift_mu 2.5e-5 --feddrift_beta 0.9`, `--mixup_alpha 0.2`, and `--cosine_lr_eta_min_factor 0.003`. FedLC, label smoothing, SAM/FedSAM, FedProx, weight-power flattening, exact local steps, FedDrift state clipping, local-loss mutations after mixup, high scheduler floors, server-LR and server-momentum neighbors, weight-decay neighbors, FedDyn-alpha neighbors under the new scheduler/client-LR best, and registered architecture variants did not improve. FedAdam-style adaptive server variants are currently unsafe or ineffective at tested settings. Exact local-step training is operationally unreliable at width 2 and lower-scoring at width 1, so stop that axis under this stack. Client-LR narrowing did not confirm the raw `0.055` near miss under the default scheduler floor, but after the lower cosine floor `0.003` became best, `lr=0.0475` produced another material improvement. Keep `lr=0.0475`, default client momentum, `cosine_lr_eta_min_factor=0.003`, `aggregation_epochs=5`, `server_lr=1.875`, `server_momentum=0.35`, `weight_decay=3.5e-4`, `feddyn_alpha=1e-4`, `feddrift_mu=2.5e-5`, `feddrift_beta=0.9`, and `fedproxloss_mu=0`. The small-head architecture came close but did not beat the current best, and its weight-decay and server-LR retunes regressed, so keep `moderate_cnn` and close the small-head branch unless a later literature loop provides a stronger reason. The shared-memory validation crash at candidate width 4 is a resource-contention signal, so subsequent batches should use `PARALLEL_CANDIDATES=2` for epoch-based runs. Parallel run launches should also set unique pycache prefixes to avoid validator races.
...
Scheduler-floor/client-LR interaction found a new best at `cosine_lr_eta_min_factor=0.003 --lr 0.0475`; server-LR, server-momentum, weight-decay, and FedDyn-alpha neighbors regressed. Watchdog recommends continuing; next CLI-only continuation should probe FedDrift scalar interactions around this stack before changing code again.
