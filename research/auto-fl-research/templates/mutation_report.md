...
Local continuation: the post-improvement FedProx-off ablation `fedproxloss_mu=0` under the new `cosine_lr_eta_min_factor=0.0001625` high-water stack scored 0.920600 in 647s and was marked `discard`. Removing the proximal term regressed below the active `fedproxloss_mu=3e-5` setting and below the nearby `2.875e-5`/`3.125e-5` retests, so the small FedProx coefficient remains part of the active stack. `scripts/plateau_watchdog.py results.tsv` reported `recommendation=continue` with `scored_since_reset=27`.

Local continuation: the post-improvement near-miss combo `server_lr=1.800390625` plus client `momentum=0.92578125` under the new `cosine_lr_eta_min_factor=0.0001625` high-water stack scored 0.921600 in 861s and was marked `discard`. Combining the strongest post-improvement client-momentum shoulder with the upper server-lr shoulder did not beat the active high-water, so keep `server_lr=1.8` and `momentum=0.925` as the active pair. `scripts/plateau_watchdog.py results.tsv` reported `recommendation=continue` with `scored_since_reset=28`.

Local continuation: the post-improvement near-miss combo `server_momentum=0.47421875` plus client `momentum=0.92578125` under the new `cosine_lr_eta_min_factor=0.0001625` high-water stack scored 0.922200 in 864s and was marked `discard`. The paired lower server-momentum shoulder and upper client-momentum shoulder still fell below the active `server_momentum=0.475` and `momentum=0.925` high-water stack, so these two axes remain bracketed independently rather than forming a useful interaction. `scripts/plateau_watchdog.py results.tsv` reported `recommendation=continue` with `scored_since_reset=29`.

Local continuation: the post-improvement server-side combo `server_lr=1.800390625` plus `server_momentum=0.47421875` under the new `cosine_lr_eta_min_factor=0.0001625` high-water stack scored 0.924100 in 865s and was marked `discard`. This was the strongest post-improvement interaction after the client-momentum near miss, but it still stayed below the active `server_lr=1.8` and `server_momentum=0.475` high-water and below the 0.926300 material-improvement threshold. `scripts/plateau_watchdog.py results.tsv` reported `recommendation=continue` with `scored_since_reset=30`.

Local continuation: the post-improvement half-step server-side combo `server_lr=1.8001953125` plus `server_momentum=0.474609375` under the new `cosine_lr_eta_min_factor=0.0001625` high-water stack scored 0.921800 in 864s and was marked `discard`. Moving only halfway from the active server settings toward the stronger 0.924100 server-side miss did not preserve that shoulder, so the local FedAvgM server-lr/momentum interaction is not a useful refinement path. `scripts/plateau_watchdog.py results.tsv` reported `recommendation=continue` with `scored_since_reset=31`.

Local continuation: the post-improvement schema-max exact-step local-compute probe `local_train_steps=1000` under the new `cosine_lr_eta_min_factor=0.0001625` high-water stack scored 0.924700 in 1074s and was marked `discard`. It was the strongest exact-step result under the improved scheduler floor but still stayed below the active epoch-based 7-epoch high-water while costing substantially more runtime, so exact-step local compute remains a non-kept alternative. `scripts/plateau_watchdog.py results.tsv` reported `recommendation=literature` with `scored_since_reset=32`; stop local hyperparameter jittering and run the literature loop before selecting the next candidates.

Literature-backed reserve mutation: add default-off R-Drop client regularization via `--rdrop_alpha`. The hypothesis from Liang21 is that two stochastic dropout passes should agree, reducing local overfitting without changing DIFF uploads, model keys, data splits, or server aggregation. Implementation adds a symmetric KL term between paired logits and keeps FedProx, class-balanced loss, SAM, and `NUM_STEPS_CURRENT_ROUND` behavior intact. Source: Liang et al., "R-Drop: Regularized Dropout for Neural Networks", NeurIPS 2021, https://proceedings.neurips.cc/paper_files/paper/2021/hash/5a66b9200f29ac3fa0ae244cc2a51b39-Abstract.html.
