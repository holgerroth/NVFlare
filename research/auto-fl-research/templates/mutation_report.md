...
Scheduler-floor retry under mixup found a new best at `cosine_lr_eta_min_factor=0.003`. Watchdog reset on that material improvement and recommends continuing; next CLI-only continuation should probe conservative interactions around the new scheduler-floor best before changing code again.
