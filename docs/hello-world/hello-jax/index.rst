...
This example demonstrates how to use NVIDIA FLARE with JAX, Flax, and Optax to train an MNIST classifier using federated averaging (FedAvg). It follows the same hello-world recipe structure as ``hello-pt``, but uses a JAX client training loop and a structured Flax state dict for transport.
...
.. code-block:: text
   ...
   |-- prepare_model.py  # helper that writes the initial structured checkpoint
...
This example uses the `MNIST <https://www.tensorflow.org/datasets/catalog/mnist>`_ dataset. ``prepare_data.py`` downloads the raw MNIST files once and converts them into ``.npy`` files. Each client then loads from that prepared cache.
...
This example uses the JAX ``FedAvgRecipe`` wrapper. The JAX parameter tree is converted to and from a structured Flax state dict, so the transport boundary stays close to the way JAX and Flax users normally serialize model parameters.

Before running the job, prepare two resources:

- The initial structured checkpoint is generated by ``prepare_model.py`` in Flax msgpack format and passed to ``FedAvgRecipe`` through ``initial_ckpt``.
...
.. code-block:: bash
   ...
   python prepare_model.py --output /path/to/initial_model.msgpack
...
.. code-block:: bash
   ...
   python job.py --initial_ckpt /path/to/initial_model.msgpack --data_dir /path/to/mnist

Output Summary
--------------

- **Initialization**: ``BaseModelController`` starts the FedAvg workflow, loads the initial structured checkpoint, and writes simulation output under ``/tmp/nvflare/simulation/hello-jax``.
...
- **Completion**: FedAvg finishes after 3 rounds, persists the final JAX checkpoint to ``/tmp/nvflare/simulation/hello-jax/server/simulate_job/models/server.msgpack``, and reports the simulation result directory at ``/tmp/nvflare/simulation/hello-jax``.
